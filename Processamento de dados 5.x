...
scp42 <- read.table(paste("A:/OneDrive - Instituto Superior de Engenharia do Porto/Doutoramento/Heuristicas/R/Meta-RaPS/SCP-Instances/scp5",SaveFile,".txt",sep=""), sep = "", header = FALSE, fill = T) 
#scp42 <- read.table("A:/OneDrive - Instituto Superior de Engenharia do Porto/Doutoramento/Heuristicas/R/Meta-RaPS/SCP-Instances/scp59.txt", sep = "", header = FALSE, fill = T) 

scp42W <- scp42[2:168,]
scp42W <- c(t(scp42W))
scp42W <- scp42W[1:2000]
rownames(scp42W) <- NULL

scp42A <- scp42[169:nrow(scp42),]
...
for (i in 6:nrow(scp42A)){
  ...
  if (       scp42A[i,1]>max(scp42A[i-1,],na.rm=T)  &&  is.na(scp42A[i-4,12])==FALSE  &&  is.na(scp42A[i-3,12])==FALSE  &&  is.na(scp42A[i-2,12])==FALSE  &&  is.na(scp42A[i-1,12])==FALSE  &&  is.na(scp42A[i-5,12])==TRUE){
...
save.image(file = paste("X5",SaveFile,".RData",sep = ""))
